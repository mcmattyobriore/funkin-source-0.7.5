...
var AL = {
  ...
  _nextId: 1,
  newId: function () {
    if (AL.freeIds.length > 0) {
      return AL.freeIds.pop();
    } else {
      return AL._nextId++;
    }
  },
...
function _alcCaptureOpenDevice(pDeviceName, requestedSampleRate, format, bufferFrameCapacity) {
  ...
  function onError(mediaStreamError) {
...
function _alcCaptureSamples(deviceId, pFrames, requestedFrameCount) {
  ...
  function setF32Sample(i, sample) {
    HEAPF32[pFrames + i * 4 >> 2] = sample;
  }
  function setI16Sample(i, sample) {
    HEAP16[pFrames + i * 2 >> 1] = sample;
  }
  ...
  if (Math.floor(fratio) == fratio) {
    var i = 0;
    for (var frame_i = 0; frame_i < requestedFrameCount; ++frame_i) {
...
function _clock() {
  ...
  return (Date.now() - _clock.start) * 1000 | 0;
}
...
function _emscriptenfte_pcm_loadaudiofile(ctx, callback, dataptr, datasize, snd_speed) {
  const successcb = function (buffer) {
    ...
    const outptr = _malloc(frames * 2 * chans);
    if (!outptr) {
      return;
    }
    const dst = HEAP16.subarray(outptr >> 1, outptr + frames * 2 * chans >> 1);
    for (let c = 0; c < buffer.numberOfChannels; c++) {
      const src = buffer.getChannelData(c);
      for (let f = 0; f < frames; f++) {
        dst[f * chans + c] = src[f] * 16384;
      }
    }
...
function _emscriptenfte_xr_setup(mode) {
  ...
  if (mode == -2) {
    if (FTEC.xrsupport & 4) {
      mode = 2;
    } else if (FTEC.xrsupport & 2) {
      mode = 1;
    } else if (FTEC.xrsupport & 1) {
      mode = 0;
    }
  }
  if (mode == -1) {
...
function _glUniform2fv(location, count, value) {
  ...
  if (count <= 144) {
    var view = miniTempWebGLFloatBuffers[count * 2 - 1];
...
function _glUniform3fv(location, count, value) {
  ...
  if (count <= 96) {
    var view = miniTempWebGLFloatBuffers[count * 3 - 1];
...
function _glUniform4fv(location, count, value) {
  ...
  if (count <= 72) {
    var view = miniTempWebGLFloatBuffers[count * 4 - 1];
    var heap = HEAPF32;
    value >>= 2;
...
function _glUniformMatrix3fv(location, count, transpose, value) {
  ...
  if (count <= 32) {
    var view = miniTempWebGLFloatBuffers[count * 9 - 1];
...
function _glUniformMatrix4fv(location, count, transpose, value) {
  ...
  if (count <= 18) {
    var view = miniTempWebGLFloatBuffers[count * 16 - 1];
    var heap = HEAPF32;
    value >>= 2;
